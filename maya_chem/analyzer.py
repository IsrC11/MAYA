...
class MayaAnalyzer:
    ...
    def visualize(self, show: bool = True, save_prefix: str | None = None, title:str = 'Chemical Space', heatmap_title: str = 'Tanimoto Heatmap', interactive_mode: bool = False):
        ...
        if interactive_mode:
            ...
            try:
                app = molplotly.add_molecules(fig=None, df=self.data, smiles_col=self.config.data['smiles_col'], title_col=self.config.data['id_col'], color_col='MolWt' if 'MolWt' in self.data.columns else None, captions_cols=[self.config.data['smiles_col'], x_col, y_col])
                app.run_server(mede='inline', port=8060, debug=False)
                fig2 = app
