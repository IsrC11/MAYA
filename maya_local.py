# -*- coding: utf-8 -*-
"""MAYA_V3_MAP4_LOCAL.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1_N_j6gOr4KXiI7u6jsCO4-d1GsSyu0BV
...
from rdkit.Chem import AllChem, Draw, PandasTools, Scaffolds, Descriptors, Descriptors3D
...
def chemical_multiverse(dataset=None, ID=None, smiles_column_name=None, target_activities=[None],
   ...
   def similarity_calc(smi1:str, smi2:str, method:str='tanimoto', fp_type:str='MACCS')->list:
        ...
        if fp_type == 'MACCS':
            ...
            fp2 = AllChem.GetMorganFingerprintAsBitVect(mol2, radius, nBits=2048)
             ...
             fp1 = map_4._calculate_single_mol_fingerprint(mol1)
             fp1 = numpy_to_bitvect(fp1)

             fp2 = map_4._calculate_single_mol_fingerprint(mol2)
             fp2 = numpy_to_bitvect(fp2)
   ...
   def calculate_all_similarities_blocks(fp_type:str='MACCS', method:str='tanimoto')->pd.DataFrame:
        ...
        for i_start in range(0, num_smiles, block_size):
            ...
            for j_start in range(0, num_smiles, block_size):
                ...
                block_similarities = Parallel(n_jobs=n_jobs)(delayed(similarity_calc)(smiles[i], smiles[j], method=method, fp_type=fp_type) for i, j in block_pairs)
  ...
  # If the user sets ECFP to true, this section will retrieve similarity_calc to generate a similarity matrix using the Extended Connectivity Fingerprint(ECFP). It is necessary to specify the radius to calculate either ECFP4 or ECFP6
   ...
   if signaturizer_code is not None:
    ...
    for code in signaturizer_code:
      ...
      if vPCA is True:
              ...
              # Plot configuration
              fig_pca_mechanisim = px.scatter(dataset_signatures_pca, x='component1',y='component2', color=metric_name, color_continuous_scale=palette, title=(f'PCA - {signaturizer_name.get(code)}'), labels={'component1': f'Component 1 ({exp_variance_signatures[0]*100:.3}%)', 'component2': f'Component 2 ({exp_variance_signatures[1]*100:.3}%)'},
               width=1200,
               height=900,
               #range_color=[min_value, max_value]
                                              )
   ...
   if MAP4 is True:
               ...
               matrix_MAP4_df = calculate_all_similarities_blocks(fp_type='MAP4', method='tanimoto')
               data_MAP4 = matrix_MAP4_df.iloc[:,0:].values
              ...
              # This section performs PCA model training and calculates the principal components.
               if vPCA is True:
                      ...
                      #fig_pca_ECFP.show()
                      #app_maker.run(port=8074)
                      #serve_kernel_port_as_iframe("localhost")
                      app_maker_MAP4_pca.run(port=8074)
               ...
               if t_SNE is True:
                      ...
                      #fig_tsne_ECFP.show()
                      #app_maker.run(port=8076)
                      #serve_kernel_port_as_iframe("localhost")
                      app_maker_ecfp_MAP4.run(port=8076)

"""# Example"""

chemical_multiverse(dataset='/home/difacquim/Desktop/Israel /data_test.csv', smiles_column_name='Smiles', signaturizer_code=None, ID='Cmpd_ID', target_activities=['Promiscuity_1uM'], perplexity=20)