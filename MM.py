...
Original file is located at
    https://colab.research.google.com/drive/1o7zS_LuOP8Hn7iogj6yAgVCh5tnxSTbu
"""
...
def chemical_multiverse(dataset=None, ID=None, smiles_column_name=None, target_activities=[None], vPCA=True, t_SNE=True, MACCS=True, ECFP=True, radius=3, druglikeness_descriptors=True, signaturizer_code=['B1','B4', 'C1', 'C2', 'E4', 'E5'], palette='RdBu_r', perplexity=33, size_point=12.0, size_point_representation='normal_desviation', evaluation_metric='mpIC50_value', metric_name='mpIC50',  point_shape='circle'):
   ...
   def parallel_pretreatment(data_frame:pd.DataFrame, smiles_column_name, n_jobs=10)->pd.DataFrame:
       '''
   ...
   def calculate_all_similarities(fp_type:str='MACCS', method:str='tanimoto')->pd.DataFrame:
        ...
        #smiles_pairs = [(smi1, smi2) for i, smi1 in enumerate(smiles) for smi2 in smiles[i+1:]]

        similarities = Parallel(n_jobs=10)(delayed(similarity_calc)(smiles[i], smiles[j], method=method, fp_type=fp_type) for i, j in pairs)
   ...
   def configure_plot_layout(fig:plt, data_frame:pd.DataFrame)->plt:
   ...
   def signaturizer_calculation(smiles_list:pd.DataFrame, signaturizer_code:list)->pd.DataFrame:
        ...
        signaturizer_mapping = {'B1': CCSpace.B1, 'B4': CCSpace.B4, 'C1': CCSpace.C1, 'C2': CCSpace.C2, 'E4': CCSpace.E4, 'E5': CCSpace.E5}
